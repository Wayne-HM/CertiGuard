...
export function ResultDisplay({ result, onVerifyAnother }: ResultDisplayProps) {
  ...
  const downloadReport = async () => {
    ...
    try {
      ...
      ctx.fillText("CERTIGUARD VERIFICATION REPORT", 100, 150)
      
      ctx.fillStyle = result.isValid ? "#000000" : "#666666"
      ctx.font = "bold 40px 'Helvetica Neue', Helvetica, Arial, sans-serif"
      ctx.fillText(result.isValid ? "STATUS: VERIFIED" : "STATUS: UNVERIFIED", 100, 220)

      // Details
      ctx.fillStyle = "#000000"
      ctx.font = "bold 30px 'Helvetica Neue', Helvetica, Arial, sans-serif"
      ctx.fillText("CANDIDATE NAME", 100, 400)
      ctx.font = "normal 50px 'Helvetica Neue', Helvetica, Arial, sans-serif"
      ctx.fillText(result.name || "N/A", 100, 460)

      ctx.font = "bold 30px 'Helvetica Neue', Helvetica, Arial, sans-serif"
      ctx.fillText("COURSE TITLE", 100, 560)
      ctx.font = "normal 50px 'Helvetica Neue', Helvetica, Arial, sans-serif"
      ctx.fillText(result.course || "N/A", 100, 620)

      ctx.font = "bold 30px 'Helvetica Neue', Helvetica, Arial, sans-serif"
      ctx.fillText("PLATFORM", 100, 720)
      ctx.font = "normal 40px 'Helvetica Neue', Helvetica, Arial, sans-serif"
      ctx.fillText(result.platform || "N/A", 100, 780)

      // QR Code
      const qrImg = new Image()
      qrImg.src = qrDataUrl
      await new Promise(resolve => qrImg.onload = resolve)
      ctx.drawImage(qrImg, W - 400, H - 450, 300, 300)
